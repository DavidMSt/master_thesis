...
from robot.lib import LCD_2inch
...
def prepare_files():
    ...
    if not base_path.exists():
        ...
        for filename in files:
            command = (
                        "curl --output ~/.cache/xgo-toolkit/model/%s -L https://github.com/jozolab/xgo-toolkit/raw/refs/heads/master/model/%s >/dev/null 2>&1"
                        % (filename, filename,))
            os.system(command)
    ...
    def run(self, img, ):
        ...
        if len(datas) > 0:
            ...
            for b in boxes:
                ...
                s = {'classes': self.classes[cls_index], 'score': '%.2f' % obj_score,
