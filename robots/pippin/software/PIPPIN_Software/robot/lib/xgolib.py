...
class XGO():
    ...
    def rider_height(self, data):
        XGOorder["BODY_HEIGHT"][1] = conver2u8(data,255)
        self.__send("BODY_HEIGHT")
    ...
    def test(self):
        ...
        self.__read(0x03, 1)
        if self.__unpack():
            x = int(self.rx_data[0])
        return x

    def brute_force_unknown_commands(self, delay=0.2):
        """
        Brute-force all possible command bytes with a 0x00 payload to try discovering
        undocumented commands that may stop controllers or perform unknown functions.

        Args:
            xgo (XGO): An instance of the XGO robot class.
            delay (float): Time to wait between commands.
        """
        print("Starting brute-force command scan...")

        for cmd in range(0x00, 0x100):
            # Construct a raw message based on the robot's protocol
            # Format: [0x55, 0x00, len, mode, cmd, data..., checksum, 0x00, 0xAA]
            mode = 0x01
            data = [0x00]  # single-byte payload
            length = len(data) + 0x08
            checksum = (length + mode + cmd + sum(data)) % 256
            checksum = 255 - checksum
            packet = [0x55, 0x00, length, mode, cmd] + data + [checksum, 0x00, 0xAA]

            print(f"Trying command 0x{cmd:02X} with payload [0x00] -> packet: {packet}")
            self.ser.write(bytearray(packet))
            time.sleep(delay)  # Give some time to observe the effect

            # Optionally, you could read and print responses here
            # if xgo.__unpack(): print(f"Response: {xgo.rx_data[:xgo.rx_COUNT+1]}")
