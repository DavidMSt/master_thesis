...
def main():
    ...
    psi1 = math.radians(180)
    x1_true = np.array([0.0, 0.0, math.sin(psi1), math.cos(psi1)])

    # Agent 2 absolute state:
    psi2 = math.radians(180)  # 135° in radians
    ...
    r21_true = np.hstack((r21_true_pos, math.sin(psi1-psi2), math.cos(psi1-psi2)))


    # Full true state vector:
    # [Agent1; Agent2 relative; Agent3 relative]
    X_true = np.hstack((x1_true, r21_true))
    ...
    X_est = np.hstack((x1_est, r21_est))
    ...
    sigma12 = np.array([1e-9, 1e-9, 1e-9, 1e-9])
